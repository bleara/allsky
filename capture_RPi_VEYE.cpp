...
{
	system("veye_mipi_i2c.sh -w -f videoformat -p1 PAL &");	
	system("veye_mipi_i2c.sh -w -f mshutter -p1 0x52 &");	
	system("veye_mipi_i2c.sh -w -f denoise -p1 0x0F &");	
	system("veye_mipi_i2c.sh -w -f agc -p1 0x0F &");	
	system("veye_mipi_i2c.sh -w -f lowlight -p1 0x11 &");	
	system("veye_mipi_i2c.sh -w -f daynightmode -p1 0xFE &");	
	system("veye_mipi_i2c.sh -w -f brightness -p1 0x32 &");	
	usleep( 1000);
...
void RPi_VEYE_Capture(int asiAutoFocus, int asiAutoExposure, int asiExposure, int asiAutoGain, int asiAutoAWB, double asiGain, int bin, double asiWBR, double asiWBB, int asiRotation, int asiFlip, int asiGamma, int asiBrightness, int quality, const char* fileName, int time, int showDetails, const char* ImgText, int fontsize, int fontcolor, int background, int darkframe)
{
	...
	string kill = "ps -ef|grep veye_raspistill| grep -v color|awk '{print $2}'|xargs kill -9 1> /dev/null 2>&1";
...
*/
	...
	string command = "nice veye_raspistill --nopreview  --output " + ss.str() + " --burst ";
...
*/
	...
	int asiAutoGain       = 0;
	int asiAutoAWB        = 0;
	int delay             = 10000;   // Delay in milliseconds. Default is 10ms
	int daytimeDelay      = 30000; // Delay in milliseconds. Default is 15 seconds
	double asiWBR         = 2.5;
	double asiWBB         = 2;
	int asiGamma          = 50;
	...
	int asiFlip           = 0;
	int asiRotation       = 0;
	char const *latitude  = "53.90N"; //GPS Coordinates of Skipton, UK where this code was altered
	char const *longitude = "2.12W";
	char const *angle  	  = "-18"; // angle of the sun with the horizon (0=sunset, -6=civil twilight, -12=nautical twilight, -18=astronomical twilight)
	//int preview           = 0;
	int time              = 0;
	int showDetails       = 0;
	int darkframe         = 0;
	...
	printf("\n");
	printf("%s ******************************************\n", KGRN);
	printf("%s *** Allsky Camera Software v0.6a | 2021 ***\n", KGRN);
	printf("%s ******************************************\n\n", KGRN);
	printf("\%sCapture images of the sky with a Raspberry Pi and an ZWO ASI or RPi HQ or VEYE camera\n", KGRN);
	printf("\n");
...
*/
	...
	init_params(); // set default parameters for veye camera

	stringstream ss;

// Set exposure via i2c
	printf("asiExposure: %x\n", asiExposure);
	...
	if (asiAutoExposure)
	{
		shutter = "veye_mipi_i2c.sh -w -f mshutter -p1 0x40 &";
	}
	...
	else if (asiExposure)
	{
		ss.str("");
		ss << asiExposure;
		shutter = "veye_mipi_i2c.sh -w -f mshutter -p1 " + ss.str() + " &";
	}

	// Add exposure time setting to i2c command string
	system(shutter.c_str());	
	printf("mshutter command: %s\n", shutter.c_str());
	...
	ss.str("");
	ss << asiFlip;
	flip = "veye_mipi_i2c.sh -w -f mirrormode -p1 " + ss.str() + " &";

	// Add mirrormode i2c command string
	system(flip.c_str());	
	printf("mirrormode command: %s\n", flip.c_str());
	...
	if (asiGamma)
	{
		ss.str("");
		ss << asiGamma;
		saturation = "veye_mipi_i2c.sh -w -f saturation -p1 " + ss.str() + " &";
	}

	// Add exposure time setting to i2c command string
	system(saturation.c_str());	
	printf("saturation command: %s\n", saturation.c_str());
	...
	ss.str("");
	ss << asiBrightness;
	brightness = "veye_mipi_i2c.sh -w -f brightness -p1 " + ss.str() + " &";

	// Add brightness i2c command string
	system(brightness.c_str());	
	printf("brightness command: %s\n", brightness.c_str());
...
// printf("Daytimecapture: %d\n", daytimeCapture);
		...
		if (dayOrNight == "DAY")
		{
			...
			if (endOfNight == true)
			{
				...
				// Add exposure time setting to i2c command string
				system("veye_mipi_i2c.sh -w -f mshutter -p1 0x40 &");	
				// Add Colour / mono setting to i2c command string
				system("veye_mipi_i2c.sh -w -f daynightmode -p1 0xFF &");	
